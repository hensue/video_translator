...
print("hf token------------", os.getenv("YOUR_HF_TOKEN"))


title = "<center><strong><font size='7'>🎥 Video Translator 🎥</font></strong></center>"
video_select = "<center><font size='3'>Import your video using a YouTube link or vfrom Files</font></center>"
language_select = "<center><font size='3'>Select the original and final language</font></center>"
normal_setting = "<center><font size='3'>Normal Settings</font></center>"
# news = """ ## 📖 News
...
<h2>1. Upload a video or provide a video link.</h2>

<h2>2. See the tab labeled 'Help' for instructions on how to use it. 🚀 Let's start having fun with video translation! 🚀</h2>
"""
...
4. 📖 In addvanced settings you can choose the model, compute type and so on.

5. 🚀 Press the '**Translate**' button to obtain the results.
"""
...
def translate_from_video(
    ...
    video_output = f"video_{datetime.datetime.now().timestamp()}.mp4"
    ...
    if os.path.exists(video):
        ...
        for i in range (120):
            ...
            #     os.system('del intermediate.aac') # only for demo
            ...
            if i == 119:
              ...
              return
        # os.system('del intermediate.aac')
    else:
        if preview:
    ...
    print("-------------------------------\n", SOURCE_LANGUAGE, WHISPER_MODEL_SIZE, device, compute_type)
    ...
    os.system(f"del {Output_name_file}")

    progress(0.90, desc="Creating final translated video...")

    create_translated_audio(result_diarize, audio_files, Output_name_file)

    os.system(f"del {mix_audio}")
...
#### video
    with gr.Tab("Audio Translation for a Video"):
        with gr.Row():
            with gr.Column():
                with gr.Group():
                #video_input = gr.UploadButton("Click to Upload a video", file_types=["video"], file_count="single") 
                    gr.Markdown(video_select)
                    blink_input = gr.Textbox(label="Video link", placeholder="YouTube Link...")
                    gr.HTML("<hr style='margin-bottom:0'>")
                    video_input = gr.File(label="Video") 
                # line_ = gr.HTML("<hr></h2>")
                #link = gr.HTML()
                #video_input.change(submit_file_func, video_input, [video_input, link], show_progress='full')
                with gr.Group():
                    gr.Markdown(language_select)
                ...
                # line_ = gr.HTML("<hr></h2>")
                with gr.Group():
                    gr.Markdown(normal_setting)
                    # gr.Markdown("Select how many people are speaking in the video.")
                    min_speakers = gr.Slider(1, MAX_TTS, label="min_speakers", step=1, visible=False, interactive=True)
                    max_speakers = gr.Slider(1, MAX_TTS, value=2, step=1, label="Max speakers", interactive=True)
                ...
                with gr.Column():
                      with gr.Accordion("Advanced Settings", open=False):
                          ...
                          AUDIO_MIX = gr.Dropdown(['Adjusting volumes and mixing audio'], value='Adjusting volumes and mixing audio', label = 'Audio Mixing Method', info="Mix original and translated audio files to create a customized, balanced output with two available mixing modes.")

                          gr.HTML("<hr style='margin-bottom:0'>")
                          gr.Markdown("Default configuration of Whisper.")
                        #   WHISPER_MODEL_SIZE = gr.Dropdown(['tiny', 'base', 'small', 'medium', 'large-v1', 'large-v2'], default=whisper_model_default, label="Whisper model")
                          WHISPER_MODEL_SIZE = gr.Dropdown(['large-v1', 'large-v2'], value=whisper_model_default, label="Whisper model", visible=True, interactive= True)
                          batch_size = gr.Slider(1, 32, label="Batch size", step=1)
                          compute_type = gr.Dropdown(list_compute_type, value=compute_type_default, label="Compute type", visible=True, interactive= True)

                          gr.HTML("<hr style='margin-bottom:0'>")
                          today = datetime.datetime.now().timestamp()
                          output_filename = f"video_{today}.mp4"
                          VIDEO_OUTPUT_NAME = gr.Textbox(label="Translated file name" ,value=output_filename, info="The name of the output file")
            ...
            with gr.Column(variant='compact'):
                ...
                line_ = gr.HTML("<hr style='margin-bottom:0'>")
...
### link
    ...
    #             blink_input = gr.Textbox(label="Media link.", info="Example: www.youtube.com/watch?v=g_9rPvbENUw", placeholder="URL goes here...")
                ...
                #           gr.HTML("<hr></h2>")
                #           gr.Markdown("Default configuration of Whisper.")
                #         #   bWHISPER_MODEL_SIZE = gr.Dropdown(['tiny', 'base', 'small', 'medium', 'large-v1', 'large-v2'], default=whisper_model_default, label="Whisper model")
                #           bWHISPER_MODEL_SIZE = gr.Dropdown(['large-v1'], value=whisper_model_default, label="Whisper model", visible=True, interactive= True)
                        
                #           bbatch_size = gr.Slider(1, 32, default=16, label="Batch size", step=1)
                #           bcompute_type = gr.Dropdown(list_compute_type, value=compute_type_default, label="Compute type", visible=True, interactive= True)
    ...
    with gr.Tab("Help"):
        ...
        # embed_html = '<video width="320" height="240" controls autoplay><source src="/Video.mp4" type="video/mp4"></video>'
        # gr.HTML(embed_html)
    # gr.Video(
